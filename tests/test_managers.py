...
class ManagerTests(unittest.TestCase):
    ...
    def test_header_lookup_and_backfill_write_only_empty_manager_cells(self):
        ...
        with patch.object(storage,'connected',return_value=True), patch.object(managers,'default_id',return_value='default'), patch.object(managers,'reservation_sheet',return_value=(sheet,rows,11)):
            managers.backfill()
        sheet.batch_update.assert_called_once_with([{'range':'L2','values':[['default']]}],value_input_option='RAW')
