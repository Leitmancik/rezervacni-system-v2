...
def call(action, **payload):
    ...
    if not url.startswith('https://script.google.com/macros/s/') or not url.endswith('/exec'):
        raise StorageError('Neplatná adresa služby úklidu.')
    ...
    try:
        ...
        raise StorageError('Výsledek operace nelze ověřit. Obnovte data a zkuste to znovu; nabídky se nevytvoří duplicitně.') from None
